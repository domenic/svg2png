...
module.exports = (sourceBuffer, options) => {
    return Promise.resolve().then(() => { // catch thrown errors
        const cp = childProcess.execFile(getPhantomJSPath(options), getPhantomJSArgs(options), { maxBuffer: Infinity });
...
function getPhantomJSPath(options) {
    if(options !== undefined && options.phantomjsPath !== undefined) {
        return path.resolve(options.phantomjsPath);
    } else {
        return phantomjsCmd;
    }
}

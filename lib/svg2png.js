...
module.exports = (sourceBuffer, options) => {
    return Promise.resolve().then(() => { // catch thrown errors
        const phantomPath = options.phantomPath || phantomjsCmd;
        const cp = childProcess.execFile(phantomPath, getPhantomJSArgs(options), { maxBuffer: Infinity });
...
module.exports.sync = (sourceBuffer, options) => {
    const phantomPath = options.phantomPath || phantomjsCmd;
    const result = childProcess.spawnSync(phantomPath, getPhantomJSArgs(options), {
...
function getPhantomJSArgs(options = {}) {
    const converterPath =  options.converterPath || converterFileName;
